...
const attachSourceOptions = [
  { value: "{receiver_phone}", label: "闲鱼联系电话" },
  { value: "{receiver_name}", label: "闲鱼收货人" },
  { value: "{receiver_address}", label: "闲鱼收货地址" },
  { value: "{receiver_city}", label: "闲鱼收货城市" },
  { value: "{buyer_id}", label: "闲鱼买家 ID" },
  { value: "{order_id}", label: "闲鱼订单号" },
  { value: "{spec_value}", label: "闲鱼规格值" },
] as const;
...
const Rules: React.FC<RulesProps> = ({
  ...
  const [debouncedAutomationSearch, setDebouncedAutomationSearch] =
    useState("");
  // [automationTriggerFilter, 解构得到当前 Hook 返回的状态和操作函数。
  const [automationTriggerFilter, setAutomationTriggerFilter] = useState<
  ...
  // fulfillmentInstances 是自动发货内可选的已配置外部货源。
  const [fulfillmentInstances, setFulfillmentInstances] = useState<
    FulfillmentInstance[]
  >([]);
  // validatingVariant 是当前正在远程校验商品的发货内容下标。
  const [validatingVariant, setValidatingVariant] = useState<number | null>(
    null,
  );
  ...
  const {
    ...
    displayVariants,
    openAutomationRule,
    openNewAutomationRule,
    handleTriggerChange,
    handleAutomationItemChange,
    updateVariant,
    updateAdjustPriceTarget,
    updateAdjustPriceNotifyText,
    appendDeliveryContent,
    handleSaveAutomationRule,
    handleDeleteAutomation,
    handleToggleAutomation,
    handleResolveRunIssue,
    handleResolveDeferredIssue,
    handleAddReplyRule,
    handleSaveReplyRule,
    handleDeleteReply,
    openDefaultReplyModal,
    handleSaveDefaultReply,
  ...
  const validateFulfillmentVariant = async (index: number): Promise<void> => {
    // variant 是当前待校验的外部发货内容。
    const variant = displayVariants[index];
    // goodsID 是从纯数字或链接中提取的货源商品主键。
    const goodsID = fulfillmentGoodsID(variant?.goods_ref || "");
    if (!variant?.instance_id) return alert("请先选择货源站");
    if (!goodsID) return alert("请填写正确的商品 ID 或 goods?id=4366 链接");
    setValidatingVariant(index);
    try {
      // product 是货源站返回的商品详情和直充字段。
      const product = await getFulfillmentProduct(variant.instance_id, goodsID);
      // attachValues 保留已经配置的值，并为新发现的货源字段建立空输入项。
      const attachValues = parseJSONObject(variant.attach_json);
      for (const field /* field 是当前货源返回的直充字段定义。 */ of product.attach ||
        []) {
        if (!(field.key in attachValues)) attachValues[field.key] = "";
      }
      updateVariant(index, {
        goods_id: goodsID,
        goods_ref: String(goodsID),
        goods_name: product.goods_name,
        goods_type: product.goods_type,
        goods_price: product.goods_price || "",
        ...recommendedExternalPricing(product.goods_price || ""),
        attach_fields: product.attach || [],
        attach_json: JSON.stringify(attachValues, null, 2),
        product_verified: true,
      });
    } catch (/* error 是单商品校验失败原因。 */ error) {
      updateVariant(index, { product_verified: false });
      alert("商品校验失败：" + (error as Error).message);
    } finally {
      setValidatingVariant(null);
    }
  };

  /** updateAttachField 更新一个货源字段绑定的闲鱼订单变量或固定值。 */
  const updateAttachField = (
    index: number,
    key: string,
    value: string,
  ): void => {
    // variant 是当前直充发货内容。
    const variant = displayVariants[index];
    // values 是当前发货内容的完整 attach 对象。
    const values = parseJSONObject(variant?.attach_json);
    values[key] = value;
    updateVariant(index, { attach_json: JSON.stringify(values, null, 2) });
  };

  useEffect(
    /* 当前回调同步 React 副作用和资源生命周期。 */ () => {
      // timer 定时器。
  ...
  useEffect(
    /* 当前回调处理异步操作结果。 */ () => {
      void loadReferenceData().catch(
        /* 当前回调处理异步操作结果。 */ (error) =>
          console.error("加载规则参考数据失败", error),
      );
      void listFulfillmentInstances()
        .then(setFulfillmentInstances)
        .catch(
          /* error 是货源实例参考数据失败原因。 */ (error) =>
            console.warn("加载外部货源失败，不阻断本地卡密规则", error),
        );
    },
    [loadReferenceData],
  );
  ...
  const visibleAutomationRules = useMemo(
    /* 当前回调处理集合中的单个元素。 */ () =>
      automationRules.filter(
        /* 当前回调处理集合中的单个元素。 */ (rule) =>
          !selectedAccountId || rule.cookie_id === selectedAccountId,
      ),
  ...
  const automationPageNumbers = useMemo(
    /* 当前回调计算并缓存派生数据。 */ () => {
      ...
      const first = Math.max(
        1,
        Math.min(automationPage - 2, automationTotalPages - 4),
      );
      // last last，负责当前功能中的对应处理。
      const last = Math.min(automationTotalPages, first + 4);
      return Array.from(
        { length: last - first + 1 },
        /* 当前回调处理用户交互或异步状态变化。 */ (_, index) => first + index,
      );
    },
  ...
  return (
    <div className="min-w-0 space-y-8 animate-fade-in">
      <div className="flex flex-col xl:flex-row justify-between xl:items-end gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-2 font-medium">
            系统通知卡片只进入自动化判断；买家消息进入关键词、默认或 AI 回复。
          </p>
        </div>
        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3">
          ...
          <button
            onClick={
              activeTab === "automation"
                ? /* 当前回调处理用户交互或异步状态变化。 */ () =>
                    openNewAutomationRule("order_paid")
                : activeTab === "reply"
                  ? handleAddReplyRule
                  : /* 当前回调处理用户交互或异步状态变化。 */ () =>
                      void openDefaultReplyModal()
            }
      ...
      {activeTab === "automation" &&
        (visibleAutomationIssues.runs.length > 0 ||
          visibleAutomationIssues.pending_tasks.length > 0) && (
          <AutomationIssuePanel
            runs={visibleAutomationIssues.runs}
            pendingTasks={visibleAutomationIssues.pending_tasks}
            onResolveRun={
              /* 当前回调处理用户交互或异步状态变化。 */ (id, resolution) =>
                void handleResolveRunIssue(id, resolution)
            }
            onResolveDeferredTask={
              /* 当前回调处理用户交互或异步状态变化。 */ (id, resolution) =>
                void handleResolveDeferredIssue(id, resolution)
            }
          />
        )}
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs opacity-75 mt-1 leading-5">
                              {meta.description}
                            </div>
                          </div>
                        </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-sm font-black text-gray-900">
                          {automationTriggerCounts[trigger] || 0}
                        </span>
                      </div>
                    );
                  },
                ...
                <div className="relative xl:w-52">
                  ...
                  <select
                    ...
                    onChange={
                      /* 当前回调处理用户交互或异步状态变化。 */ (event) => {
                        setAutomationTriggerFilter(
                          event.target.value as AutomationTriggerType | "",
                        );
                        setAutomationPage(1);
                      }
                    }
                    ...
                    {triggerOrder.map(
                      /* 当前回调处理集合中的单个元素。 */ (trigger) => (
                        <option key={trigger} value={trigger}>
                          {triggerMeta[trigger].shortLabel}
                        </option>
                      ),
                    )}
                  </select>
              ...
              visibleAutomationRules.map(
                /* 当前回调处理集合中的单个元素。 */ (rule) => {
                  ...
                  return (
                    <article
                      key={rule.id}
                      className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm hover:shadow-lg transition-all"
                    >
                      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        <div className="flex items-start gap-4 min-w-0">
                          <div
                            className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${accentClasses(meta.accent, true)}`}
                          >
                            <Icon className="w-5 h-5" />
                          </div>
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              ...
                              <span
                                className={`px-2.5 py-1 rounded-full text-xs font-bold ${statusPill(rule.enabled)}`}
                              >
                                {rule.enabled ? "已启用" : "已禁用"}
                              </span>
                            </div>
                            <div className="flex flex-wrap gap-2 text-xs font-bold">
                              <span className="px-2.5 py-1 rounded-lg bg-gray-100 text-gray-600">
                                {meta.label}
                              </span>
                              <span className="px-2.5 py-1 rounded-lg bg-gray-100 text-gray-600">
                                {rule.item_title ||
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button
                            onClick={
                              /* 当前回调处理用户交互或异步状态变化。 */ () =>
                                handleDeleteAutomation(rule.id)
                            }
            ...
            {automationTotal > 0 && (
              <div className="flex flex-col gap-3 rounded-xl border border-gray-100 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3 text-sm font-medium text-gray-500">
                  <span>
                    第 {automationPage} / {Math.max(automationTotalPages, 1)} 页
                  </span>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {automationPageNumbers.map(
                    /* 当前回调处理集合中的单个元素。 */ (pageNumber) => (
                      <button
                        key={pageNumber}
                        type="button"
                        disabled={loading}
                        ...
                        aria-current={
                          pageNumber === automationPage ? "page" : undefined
                        }
                      >
                        {pageNumber}
                      </button>
                  ...
                  <button
                    ...
                    onClick={
                      /* 当前回调处理用户交互或异步状态变化。 */ () =>
                        setAutomationPage(
                          /* 当前回调处理用户交互或异步状态变化。 */ (page) =>
                            Math.min(automationTotalPages, page + 1),
                        )
                    }
                    className="flex h-9 w-9 items-center justify-center rounded-lg bg-gray-50 text-gray-600 transition-colors hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40"
                    aria-label="下一页"
          ...
          <div className="space-y-3">
            {replyRules.map(
              /* 当前回调处理集合中的单个元素。 */ (rule) => (
                <div
                  key={rule.id}
                  className="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-2xl border border-gray-100 bg-surface-subtle hover:bg-white hover:shadow-lg transition-all gap-4"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="px-3 py-1 bg-black text-white rounded-lg text-xs font-bold">
                        包含匹配
                      </span>
                    ...
                    <div className="bg-white p-3 rounded-xl border border-gray-100 text-sm text-gray-600 leading-relaxed">
                      {rule.type === "image" && rule.image_url
                        ? rule.image_url
                        : rule.reply_content}
                    </div>
                  </div>
      ...
      {activeTab === "default" && (
        <section className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {visibleDefaultAccounts.map(
              /* 当前回调处理集合中的单个元素。 */ (account) => {
                ...
                return (
                  <div
                    key={account.id}
                    className={`flex flex-col md:flex-row md:items-center justify-between p-5 rounded-2xl border transition-all gap-4 ${enabled ? "border-purple-100 bg-purple-50/50 hover:bg-white hover:shadow-lg" : "border-gray-100 bg-surface-subtle hover:bg-white hover:shadow-lg"}`}
                  >
                    <div className="flex items-center gap-4 min-w-0">
                      <div
                        className={`w-12 h-12 rounded-2xl flex items-center justify-center ${enabled ? "bg-purple-600 text-white" : "bg-gray-200 text-gray-400"}`}
                      >
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-lg text-xs font-bold ${enabled ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-500"}`}
                          >
                            {enabled ? "已启用" : "未启用"}
                          </span>
                          {defaultReply?.reply_once && (
                            <span className="px-2 py-0.5 rounded-lg text-xs font-bold bg-purple-100 text-purple-700">
                              只回复一次
                            </span>
                          )}
                        </div>
                        <div className="text-sm text-gray-600 line-clamp-2">
                          {enabled
                            ? defaultReply.reply_content ||
                              defaultReply.reply_image_url ||
                              "已配置默认回复"
                            : "未配置默认回复"}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3 border-t md:border-t-0 md:border-l border-gray-200 pt-4 md:pt-0 md:pl-6">
                      <button
                        onClick={
                          /* 当前回调处理用户交互或异步状态变化。 */ () =>
                            void openDefaultReplyModal(account.id)
                        }
                        className="p-2 text-gray-400 hover:text-black hover:bg-gray-100 rounded-xl transition-colors"
                        title="编辑"
                      >
                      ...
                      {enabled && (
                        <>
                          <button
                            onClick={
                              /* 当前回调处理用户交互或异步状态变化。 */ () =>
                                void handleClearDefaultReplyRecords(account.id)
                            }
                          ...
                          <button
                            onClick={
                              /* 当前回调处理用户交互或异步状态变化。 */ () =>
                                void handleDeleteDefaultReply(account.id)
                            }
                            className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors"
                            title="删除"
                          >
            ...
            {visibleDefaultAccounts.length === 0 && (
              <div className="text-center py-20 text-gray-400">暂无账号</div>
            )}
          </div>
        </section>
      )}
        ...
        createPortal(
          <div className="modal-overlay">
            <div
              ...
              style={{ maxWidth: "72rem", maxHeight: "92vh" }}
            >
              <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <h3 className="text-2xl font-black text-gray-900">
                    {editingAutomationRule.id
                      ? "编辑自动化规则"
                      : "新建自动化规则"}
                  </h3>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] min-h-0">
                <aside className="bg-slate-900 text-white p-5 overflow-y-auto">
                      ...
                      /* 当前回调处理集合中的单个元素。 */ (trigger) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start gap-3">
                              <Icon
                                className={`w-5 h-5 mt-0.5 ${selected ? "text-brand" : "text-white"}`}
                              />
                              <div>
                                <div className="font-black">{meta.label}</div>
                                <div
                                  className={`text-xs mt-1 leading-5 ${selected ? "text-gray-500" : "text-gray-400"}`}
                                >
                                  {meta.description}
                                </div>
                              </div>
                  ...
                  <div className="mt-6 rounded-2xl bg-white/5 border border-white/10 p-4">
                    ...
                    <div className="space-y-3">
                      {currentMeta.flow.map(
                        /* 当前回调处理集合中的单个元素。 */ (step, index) => (
                          <div key={step} className="flex items-center gap-3">
                            <div className="w-6 h-6 rounded-full bg-white text-slate-950 text-xs font-black flex items-center justify-center">
                              {index + 1}
                            </div>
                          ...
                          <select
                            ...
                            onChange={
                              /* 当前回调处理用户交互或异步状态变化。 */ (
                                ...
                                handleAutomationItemChange(event.target.value)
                            }
                            className="w-full ios-input px-4 py-3 rounded-xl"
                          >
                            <option value="">账号级规则（不限定商品）</option>
                            {modalAccountItems.map(
                              /* 当前回调处理集合中的单个元素。 */ (item) => (
                                <option
                          ...
                          <div className="mt-4 rounded-2xl bg-gray-50 border border-gray-100 p-4">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              <span className="px-3 py-1.5 rounded-lg bg-gray-100 text-gray-700 text-xs font-bold">
                                {selectedRuleItem.item_title ||
                                  selectedRuleItem.item_id}
                              </span>
                              <span
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold ${isMultiSpecRule ? "bg-blue-50 text-blue-700" : "bg-gray-100 text-gray-500"}`}
                              >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <input
                              type="text"
                              inputMode="decimal"
                              value={adjustPriceTarget(
                                editingAutomationRule.actions,
                              )}
                              onChange={
                                /* 当前回调处理用户交互或异步状态变化。 */ (
                                  event,
                                ) => updateAdjustPriceTarget(event.target.value)
                              }
                              placeholder="例如：9.9"
                              className="w-full ios-input px-4 py-3 rounded-xl"
                            />
                            <p className="text-xs text-gray-500 mt-2">
                              买家拍下未付款后，系统会把该笔订单价格修改为此金额（0.01
                              - 1000000 元，最多两位小数）。
                            </p>
                          </div>
                          <div>
                            ...
                            <textarea
                              value={
                                editingAutomationRule.actions?.find(
                                  /* 当前回调处理集合中的单个元素。 */ (
                                    action,
                                  ) => action.action_type === "send_text",
                                )?.message_template || ""
                              }
                              onChange={
                                /* 当前回调处理用户交互或异步状态变化。 */ (
                        ...
                        <div className="space-y-3">
                          {displayVariants.map((variant, index) => (
                            ...
                            >
                              ...
                              <div>
                                ...
                                <div className="grid gap-2 sm:grid-cols-2">
                                  <button
                                    ...
                                    className={`min-h-11 rounded-xl border px-4 text-sm font-bold ${variant.source_type !== "external" ? "border-brand bg-blue-50 text-brand" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}
                                  >
                                    本地卡密库存
                                  </button>
                                  <button
                                    ...
                                    className={`min-h-11 rounded-xl border px-4 text-sm font-bold ${variant.source_type === "external" ? "border-violet-500 bg-violet-50 text-violet-700" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}
                                  >
                                    外部货源
                                  </button>
                                </div>
                              ...
                              {variant.source_type === "external" ? (
                                <div className="space-y-3 rounded-xl border border-violet-100 bg-violet-50/40 p-4">
                                  <div className="grid gap-3 md:grid-cols-[1fr_1.3fr_auto]">
                                    <label className="text-xs font-bold text-gray-600">
                                      货源站 *
                                      <select
                                        value={variant.instance_id || 0}
                                        onChange={
                                          /* instanceChangeHandler 切换货源并清除旧校验结果。 */ (
                                            event,
                                          ) =>
                                            updateVariant(index, {
                                              instance_id: Number(
                                      ...
                                      >
                                        ...
                                        {fulfillmentInstances
                                          .filter(
                                            /* enabledFulfillmentFilter 只展示已启用货源。 */ (
                                              instance,
                                            ) => instance.enabled,
                                          )
                                          .map(
                                            /* fulfillmentOptionMapper 渲染货源选项。 */ (
                                              instance,
                                            ) => (
                                              <option
                                                key={instance.id}
                                    ...
                                    <label className="text-xs font-bold text-gray-600">
                                      商品 ID 或链接 *
                                      <input
                                        value={variant.goods_ref || ""}
                                        onChange={
                                          /* goodsRefChangeHandler 更新商品引用并要求重新校验。 */ (
                                            event,
                                          ) =>
                                            updateVariant(index, {
                                              goods_ref: event.target.value,
                                              goods_id: 0,
                                              product_verified: false,
                                            })
                                        }
                                        className="mt-2 w-full ios-input px-3 py-2.5 rounded-lg"
                                        placeholder="4366 或 goods?id=4366"
                                      />
                                    </label>
                                    <button
                                      type="button"
                                      disabled={validatingVariant === index}
                                      onClick={
                                        /* validateGoodsHandler 校验当前单个货源商品。 */ () =>
                                          void validateFulfillmentVariant(index)
                                      }
                                      className="min-h-11 self-end rounded-xl bg-violet-600 px-4 text-sm font-bold text-white disabled:opacity-60"
                                    >
                                      {validatingVariant === index
                                        ? "校验中…"
                                        : "校验商品"}
                                    </button>
                                  </div>
                                  {variant.product_verified && (
                                    <div
                                      role="status"
                                      className="rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-sm text-emerald-800"
                                    >
                                      <span className="font-bold">
                                        {variant.goods_name}
                                      </span>
                                      <span className="ml-2">
                                        {variant.goods_type === 1
                                          ? "卡密"
                                          : "直充"}{" "}
                                        · 采购价 ¥{variant.goods_price || "—"}
                                      </span>
                                    </div>
                                  )}
                                  <div className="grid gap-3 md:grid-cols-2">
                                    ...
                                    {variant.goods_type === 2 && (
                                      <div className="space-y-3 md:col-span-2">
                                        ...
                                        {(variant.attach_fields || []).map(
                                          /* attachFieldMapper 渲染货源字段与闲鱼订单字段的绑定。 */ (
                                            field,
                                          ) => {
                                            // attachValues、currentValue、selectedSource 是当前字段的保存值和数据来源选择。
                                            const attachValues =
                                              parseJSONObject(
                                                variant.attach_json,
                                              );
                                            const currentValue = String(
                                              attachValues[field.key] ?? "",
                                            );
                                            // fieldSourceOptions 优先提供与货源字段同名的闲鱼订单动态字段，再提供通用订单字段。
                                            const fieldSourceOptions = [
                                              ...
                                              {
                                                value: `{order_field:${field.name || field.key}}`,
                                                label: `闲鱼订单字段：${field.name || field.key}`,
                                              },
                                              ...attachSourceOptions,
                                            ];
                                            // selectedSource 是当前直充字段选择的数据来源。
                                            const selectedSource =
                                              fieldSourceOptions.some(
                                                /* sourceMatcher 判断当前值是否是已知订单变量。 */ (
                                                  option,
                                                ) =>
                                            ...
                                            return (
                                              <div
                                                key={field.key}
                                                className="grid gap-2 rounded-xl border border-violet-100 bg-white p-3 md:grid-cols-[1fr_190px_1.2fr] md:items-end"
                                              >
                                                <div>
                                                ...
                                                <label className="text-xs font-bold text-gray-600">
                                                  ...
                                                  <select
                                                    ...
                                                    onChange={
	                                                      /* attachSourceChangeHandler 切换聊天收集或闲鱼订单字段。 */ (
                                                        event,
                                                      ) =>
                                                        updateAttachField(
                                                          index,
                                                  ...
                                                  >
                                                    ...
                                                    {fieldSourceOptions.map(
                                                      /* attachSourceOptionMapper 渲染订单字段来源。 */ (
                                                        option,
                                                      ) => (
                                                        <option
                                                          key={option.value}
	                                                ...
	                                                {selectedSource === "{chat_input}" ? (
	                                                  <div className="rounded-lg bg-violet-50 px-3 py-2.5 text-xs font-bold leading-5 text-violet-700">
	                                                    付款后自动索取，买家回复“确认”后才提交货源站。
	                                                  </div>
	                                                ) : selectedSource ? (
                                                  <div className="rounded-lg bg-blue-50 px-3 py-2.5 text-xs font-bold text-blue-700">
                                                    下单时自动读取：
                                                    {
                                                      fieldSourceOptions.find(
                                                        /* selectedAttachSourceFinder 查找当前订单字段的中文名称。 */ (
                                                          option,
                                                        ) =>
                                                          option.value ===
                                                          selectedSource,
	                                                  ...
	                                                  <div className="rounded-lg bg-amber-50 px-3 py-2.5 text-xs font-bold text-amber-700">
	                                                    请选择账号来源
	                                                  </div>
                                                )}
                                              </div>
                                        ...
                                        <details className="rounded-xl border border-gray-200 bg-white p-3">
                                          ...
                                          <textarea
                                            rows={3}
                                            value={variant.attach_json || "{}"}
                                            onChange={
                                              /* attachChangeHandler 更新直充附加字段。 */ (
                                                event,
                                              ) =>
                                                updateVariant(index, {
                                                  attach_json:
                                            ...
                                            className="mt-3 w-full ios-input rounded-lg px-3 py-2.5 font-mono text-xs"
                                            placeholder={
                                              '{"1":"{receiver_phone}"}'
                                            }
                                          />
                                ...
                                <label className="block text-xs font-bold text-gray-600">
                                  ...
                                  >
                                    ...
                                    {cards
                                      .filter(
                                        /* enabledCardFilter 只展示可发货卡密。 */ (
                                          card,
                                        ) =>
                                          card.enabled &&
                                          (card.type !== "api" ||
                                            card.api_config?.ready === true),
                                      )
                                      .map(
                                        /* cardOptionMapper 渲染卡密选项。 */ (
                                          card,
                                        ) => (
                              ...
                              <div className="grid items-end gap-3 md:grid-cols-[140px_1fr_44px]">
                                <label className="text-xs font-bold text-gray-600">
                                  每件份数
                                  <input
                                    type="number"
                                    ...
                                    onChange={
                                      /* deliveryCountChangeHandler 更新每件采购或发送份数。 */ (
                                        event,
                                      ) =>
                                        updateVariant(index, {
                                          delivery_count: Math.max(
                                            1,
                                ...
                                <div className="flex min-h-11 flex-wrap items-center gap-3 rounded-xl bg-gray-50 px-3 py-2">
                                  ...
                                  <span className="text-xs text-gray-500">
                                    {variant.delay_override
                                      ? `本动作延时 ${variant.delay_seconds || 0} 秒`
                                      : variant.source_type === "external"
                                        ? "外部采购成功后立即发送"
                                        : "使用卡密默认延时"}
                                  </span>
                                ...
                                <button
                                  ...
                                  onClick={
                                    /* 当前回调处理用户交互或异步状态变化。 */ () =>
                                      setEditingAutomationRule({
                                        ...editingAutomationRule,
                                        variants: displayVariants.filter(
                                          /* 当前回调处理集合中的单个元素。 */ (
                                            _,
                                            variantIndex,
                                          ) => variantIndex !== index,
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              min="1"
                              value={Number(
                                reviewConfig.after_shipped_hours || 72,
                              )}
                              onChange={
                                /* 当前回调处理用户交互或异步状态变化。 */ (
                                  event,
                                  ...
                                  setEditingAutomationRule({
                                    ...editingAutomationRule,
                                    config_json: buildReviewConfig(
                                      editingAutomationRule.config_json,
                                      {
                                        after_shipped_hours: Math.max(
                                          1,
                          ...
                          <div>
                            ...
                            <input
                              ...
                              min="1"
                              value={Number(
                                reviewConfig.repeat_interval_hours || 24,
                              )}
                              onChange={
                                /* 当前回调处理用户交互或异步状态变化。 */ (
                                  event,
                                  ...
                                  setEditingAutomationRule({
                                    ...
                                    config_json: buildReviewConfig(
                                      editingAutomationRule.config_json,
                                      {
                                        repeat_interval_hours: Math.max(
                                          1,
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-4 items-end">
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mb-2">
                            数字越小优先级越高；同一账号、商品和触发条件只执行优先级最高的一条规则。
                          </p>
                          <input
                            type="number"
              ...
              <div className="px-6 py-4 border-t border-gray-100 bg-white flex gap-3">
                <button
                  ...
                  className="flex-1 px-6 py-3 rounded-2xl font-bold bg-gray-100 text-gray-700 hover:bg-gray-200"
                >
                  取消
                </button>
                <button
                  onClick={handleSaveAutomationRule}
                  className="flex-1 ios-btn-primary px-6 py-3 rounded-2xl font-bold flex items-center justify-center gap-2"
                >
                    ...
                    >
                      ...
                      {items
                        .filter(
                          /* 当前回调处理集合中的单个元素。 */ (item) =>
                            !selectedAccountId ||
                            item.cookie_id === selectedAccountId,
                        )
                        .map(
                          /* 当前回调处理集合中的单个元素。 */ (item) => (
                            <option
                      ...
                      onChange={
                        /* 当前回调处理用户交互或异步状态变化。 */ (event) => {
                          ...
                          setEditingReplyRule({
                            ...
                            reply_content:
                              type === "text"
                                ? editingReplyRule.reply_content
                                : "",
                            image_url:
                              type === "image"
                                ? editingReplyRule.image_url
                                : "",
                          });
                        }
                      }
            ...
            <div className="modal-container">
              ...
              <div className="modal-body space-y-5">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                  ...
                  >
                    <span
                      className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow-md transition-transform duration-300 block ${defaultForm.enabled ? "translate-x-7" : "translate-x-1"}`}
                    />
                  </button>
                </div>
                  ...
                  <input
                    ...
                    onChange={
                      /* 当前回调处理用户交互或异步状态变化。 */ (event) =>
                        setDefaultForm({
                          ...defaultForm,
                          reply_image_url: event.target.value,
                        })
                    }
                  ...
                  <input
                    ...
                    onChange={
                      /* 当前回调处理用户交互或异步状态变化。 */ (event) =>
                        setDefaultForm({
                          ...defaultForm,
                          reply_once: event.target.checked,
                        })
                    }
                    className="w-4 h-4 rounded"
                  />
