...
export interface RuleActionsState {
  ...
  setEditingAutomationRule: Dispatch<
    SetStateAction<Partial<ShippingRule> | null>
  >;
  ...
  handleResolveRunIssue: (
    id: number,
    resolution: "continue" | "retry" | "cancel",
  ) => Promise<void>;
...
export const useRuleActions = ({
  ...
  // automationSubmitState 保存自动化规则提交状态。
  const [automationSubmitState, setAutomationSubmitState] =
    useState<RuleSubmitState>(idleRuleSubmitState);
  // replySubmitState 保存关键词回复提交状态。
  const [replySubmitState, setReplySubmitState] =
    useState<RuleSubmitState>(idleRuleSubmitState);
  // defaultReplySubmitState 保存默认回复提交状态。
  const [defaultReplySubmitState, setDefaultReplySubmitState] =
    useState<RuleSubmitState>(idleRuleSubmitState);
  // editingAutomationRule 保存当前自动化规则草稿。
  const [editingAutomationRule, setEditingAutomationRule] =
    useState<Partial<ShippingRule> | null>(null);
  // editingReplyRule 保存当前关键词回复草稿。
  const [editingReplyRule, setEditingReplyRule] =
    useState<Partial<ReplyRule> | null>(null);
  // defaultForm 保存当前默认回复草稿。
  ...
  const selectedRuleItem = useMemo(
    /* 当前回调计算规则草稿绑定的商品。 */ () =>
      editingAutomationRule?.cookie_id && editingAutomationRule.item_id
        ? items.find(
            /* 当前回调查找匹配的商品。 */ (item) =>
              item.cookie_id === editingAutomationRule.cookie_id &&
              item.item_id === editingAutomationRule.item_id,
          )
        : undefined,
  ...
  // currentTrigger 保存当前规则草稿的触发类型。
  const currentTrigger = (editingAutomationRule?.trigger_type ||
    "order_paid") as AutomationTriggerType;
  ...
  const buildAutomationDraft = useCallback(
    /* buildDraftAction 创建规则默认草稿。 */ (
      ...
      const item = items.find(
        /* 当前回调查找规则绑定的商品。 */ (candidate) =>
          candidate.cookie_id === cookieID && candidate.item_id === itemID,
      );
      ...
      return {
        ...
        enabled: true,
        config_json:
          trigger === "review_missing_timeout" ? buildReviewConfig() : "{}",
        actions: cardActionsForTrigger(trigger),
        variants:
          trigger === "review_missing_timeout" || trigger === "order_created"
            ? []
            : [emptyVariant()],
      };
    },
  ...
  const openAutomationRule = useCallback(
    /* openRuleAction 打开已有规则草稿。 */ (rule: ShippingRule) => {
      // trigger 保存已有规则的触发类型。
      ...
      setEditingAutomationRule({
        ...
        trigger_type: trigger,
        config_json:
          trigger === "review_missing_timeout"
            ? buildReviewConfig(rule.config_json)
            : rule.config_json || "{}",
        actions: rule.actions?.length
          ? rule.actions.map(
              /* 当前回调复制自动化动作。 */ (action) => ({ ...action }),
            )
          : cardActionsForTrigger(trigger, rule.card_group_id),
        variants: rule.variants?.length
          ? rule.variants.map(
              /* 当前回调复制发货规格。 */ (variant) => ({ ...variant }),
            )
          : trigger === "review_missing_timeout" || trigger === "order_created"
            ? []
            : [emptyVariant()],
        ...
        try {
          ...
          if (rule) {
            ...
            setEditingAutomationRule({
              ...buildAutomationDraft(
                "order_paid",
                initialDeliveryTarget.cookieId,
                initialDeliveryTarget.itemId,
              ),
              item_title: item?.item_title || "",
              item_keyword: item?.item_title || initialDeliveryTarget.itemId,
              name: defaultRuleName(
                "order_paid",
                item?.item_title || initialDeliveryTarget.itemId,
              ),
            });
            setShowAutomationModal(true);
          }
  ...
  const openNewAutomationRule = useCallback(
    /* openNewAction 创建新规则草稿。 */ (
      trigger: AutomationTriggerType = "order_paid",
    ) => {
  ...
  const handleTriggerChange = useCallback(
    /* triggerAction 切换规则触发类型。 */ (trigger: AutomationTriggerType) => {
      if (!editingAutomationRule) return;
      // currentCardID 保存当前规则选中的卡密组。
      const currentCardID =
        editingAutomationRule.variants?.find(
          /* 当前回调查找当前卡密组。 */ (variant) => variant.card_id,
        )?.card_id ||
        editingAutomationRule.actions?.find(
          /* 当前回调查找发卡动作。 */ (action) =>
            action.action_type === "send_card",
        )?.card_id ||
      ...
      const itemLabel =
        ...
        editingAutomationRule.item_title ||
      ...
      setEditingAutomationRule({
        ...
        variants:
          trigger === "review_missing_timeout" || trigger === "order_created"
            ? []
            : editingAutomationRule.variants?.length
              ? editingAutomationRule.variants
              : [{ ...emptyVariant(), card_id: currentCardID }],
      });
    },
  ...
  const handleAutomationItemChange = useCallback(
    /* itemAction 切换规则绑定商品。 */ (itemID: string) => {
      if (!editingAutomationRule) return;
      // item 保存当前账号下选择的商品。
      const item = items.find(
        /* 当前回调查找切换后的商品。 */ (candidate) =>
          candidate.cookie_id ===
            (editingAutomationRule.cookie_id || selectedAccountId) &&
  ...
  const updateVariant = useCallback(
    /* variantAction 更新规则规格。 */ (
      index: number,
      patch: Partial<ShippingVariant>,
    ) => {
      setEditingAutomationRule(
        /* currentRule 保证连续输入时始终基于最新表单状态合并。 */ (
          currentRule,
        ) => {
          if (!currentRule) return currentRule;
          // currentVariants 是当前最新规则中的规格列表。
          const currentVariants = currentRule.variants?.length
            ? currentRule.variants
            : [emptyVariant()];
          // next 保存合并后的规格列表。
          const next = currentVariants.map(
  ...
  const updateAdjustPriceTarget = useCallback(
    /* adjustPriceAction 更新改价目标价格。 */ (targetPrice: string) => {
      if (!editingAutomationRule) return;
      // baseActions 保存至少包含改价动作的当前动作列表。
      const baseActions = editingAutomationRule.actions?.some(
        /* 当前回调查找改价动作。 */ (action) =>
          action.action_type === "adjust_price",
      )
      ...
      setEditingAutomationRule({
        ...
        actions: baseActions.map(
          /* 当前回调把新目标价格写入改价动作配置。 */ (action) =>
            action.action_type === "adjust_price"
              ? { ...action, config_json: buildAdjustPriceConfig(targetPrice) }
              : action,
        ),
      });
    },
  ...
  const updateAdjustPriceNotifyText = useCallback(
    /* adjustNotifyAction 更新改价提醒文案。 */ (text: string) => {
      if (!editingAutomationRule) return;
      // actions 保存当前动作列表副本。
  ...
  const appendDeliveryContent = useCallback(
    /* appendVariantAction 追加规则规格。 */ () => {
      if (!editingAutomationRule) return;
  ...
  const handleSaveAutomationRule = useCallback(
    /* saveRuleAction 保存自动化规则。 */ async () => {
      if (!editingAutomationRule || automationSubmitState.submitting) return;
      // trigger 保存当前规则触发类型。
      const trigger = (editingAutomationRule.trigger_type ||
        "order_paid") as AutomationTriggerType;
      if (!editingAutomationRule.cookie_id) return alert("请选择账号");
      // variants 保存当前规格列表。
      const variants = editingAutomationRule.variants?.length
        ? editingAutomationRule.variants
        : [];
      if (trigger !== "review_missing_timeout" && trigger !== "order_created") {
        ...
        for (const variant /* variant 是当前待校验直充 JSON 的发货内容。 */ of variants) {
          ...
          try {
            ...
            const attach = JSON.parse(variant.attach_json || "{}");
            if (!attach || typeof attach !== "object" || Array.isArray(attach))
              return alert("直充附加字段必须是正确的 JSON 对象");
            if (
              variant.goods_type === 2 &&
              (variant.attach_fields || []).some(
                /* missingAttachFieldValidator 阻止缺失的直充字段进入采购。 */ (
                  field,
                ) => !String(attach[field.key] ?? "").trim(),
              )
            )
              return alert("请为每个直充字段选择闲鱼订单数据来源或填写固定值");
          } catch {
            return alert("直充附加字段必须是正确的 JSON 对象");
          }
        }
        if (
          ...
          variants.some(
            /* 当前回调校验多规格字段。 */ (variant) =>
              !variant.spec_name.trim() || !variant.spec_value.trim(),
          )
        )
          return alert("多规格商品必须填写每一行的规格名称和规格值");
      }
      if (trigger === "review_missing_timeout") {
        // text 保存求评价动作的文案。
        const text =
          editingAutomationRule.actions?.find(
            /* 当前回调查找求评价文案动作。 */ (action) =>
              action.action_type === "send_text",
          )?.message_template || "";
        if (!text.trim()) return alert("请填写求评价文案");
      }
      // saveActions 保存实际提交的动作列表；拍下改价规则会剔除未填写文案的可选提醒动作。
      let saveActions = editingAutomationRule.actions?.length
        ? editingAutomationRule.actions
        : cardActionsForTrigger(
            trigger,
            editingAutomationRule.card_group_id || 0,
          );
      if (trigger === "order_created") {
        if (!isValidAdjustPrice(adjustPriceTarget(saveActions)))
          return alert("请填写 0.01 到 1000000 元、最多两位小数的目标价格");
        saveActions = saveActions.filter(
          /* 当前回调剔除空文案的可选提醒动作。 */ (action) =>
            action.action_type !== "send_text" ||
            Boolean(action.message_template?.trim()),
        );
      }
          ...
          : variants.map(
              /* 当前回调归一化发货规格字段。 */ (variant) => ({
                ...
                delivery_count: Math.max(
                  1,
                  Number(variant.delivery_count) || 1,
                ),
      ...
      try {
        await updateShippingRule({
          ...
          trigger_type: trigger,
          name:
            (editingAutomationRule.name || "").trim() ||
            defaultRuleName(
              trigger,
              selectedRuleItem?.item_title ||
                editingAutomationRule.item_id ||
          ...
          actions:
            trigger === "order_created"
              ? saveActions
              : editingAutomationRule.actions?.length
                ? editingAutomationRule.actions
                : cardActionsForTrigger(
                    trigger,
                    saveVariants[0]?.card_id ||
                      editingAutomationRule.card_group_id ||
                      0,
                  ),
        ...
        setAutomationSubmitState(
          /* current 保存自动化提交状态。 */ (current) =>
            finishRuleSubmission(current, succeeded),
        );
      }
    },
    [
  ...
  const handleDeleteAutomation = useCallback(
    /* deleteRuleAction 删除自动化规则。 */ async (id: string) => {
      if (!confirm("确定删除该自动化规则吗？")) return;
      try {
        await deleteShippingRule(id);
        await loadAutomationRules();
        alert("删除成功");
      } catch (/* error 表示自动化规则删除异常。 */ error) {
  ...
  const handleToggleAutomation = useCallback(
    /* toggleRuleAction 切换自动化规则状态。 */ async (rule: ShippingRule) => {
      try {
        await updateShippingRule({ ...rule, enabled: !rule.enabled });
        await loadAutomationRules();
      } catch (/* error 表示自动化规则状态更新异常。 */ error) {
  ...
  const handleResolveRunIssue = useCallback(
    /* resolveRunAction 处理自动化运行异常。 */ async (
      id: number,
      resolution: "continue" | "retry" | "cancel",
    ) => {
  ...
  const handleResolveDeferredIssue = useCallback(
    /* resolveDeferredAction 处理延迟任务异常。 */ async (
      id: number,
      resolution: "retry" | "dismiss",
    ) => {
      if (
  ...
  const handleAddReplyRule = useCallback(
    /* addReplyAction 创建关键词回复草稿。 */ () => {
  ...
  const handleSaveReplyRule = useCallback(
    /* saveReplyAction 保存关键词回复。 */ async () => {
      if (
        !editingReplyRule ||
        !selectedAccountId ||
  ...
  const openDefaultReplyModal = useCallback(
    /* openDefaultAction 加载默认回复草稿。 */ async (
      cookieID = selectedAccountId,
    ) => {
  ...
  const handleSaveDefaultReply = useCallback(
    /* saveDefaultAction 保存默认回复。 */ async () => {
      if (defaultReplySubmitState.submitting) return;
      if (!defaultForm.cookie_id) return alert("请先选择账号");
      if (
        defaultForm.enabled &&
        !defaultForm.reply_content.trim() &&
        !defaultForm.reply_image_url.trim()
      )
        return alert("启用默认回复时，请填写回复内容或图片 URL");
      ...
      try {
        ...
        alert("保存成功");
        succeeded = true;
      } catch (/* error 表示默认回复保存异常。 */ error) {
        alert("保存失败：" + (error as Error).message);
      } finally {
        setDefaultReplySubmitState(
          /* current 保存默认回复提交状态。 */ (current) =>
            finishRuleSubmission(current, succeeded),
        );
      }
    },
  ...
  const handleDeleteDefaultReply = useCallback(
    /* deleteDefaultAction 删除默认回复。 */ async (cookieID: string) => {
      if (!confirm("确定删除该账号默认回复吗？")) return;
      try {
        await deleteDefaultReply(cookieID);
        await loadDefaultReplies();
        alert("删除成功");
      } catch (/* error 表示默认回复删除异常。 */ error) {
        alert("删除失败：" + (error as Error).message);
      }
    },
    [loadDefaultReplies],
  ...
  const handleClearDefaultReplyRecords = useCallback(
    /* clearDefaultRecordsAction 清空默认回复记录。 */ async (
      cookieID: string,
    ) => {
      if (
        !confirm(
          "确定清空该账号的默认回复记录吗？清空后可重新对所有会话使用“只回复一次”。",
        )
      )
        return;
      try {
        ...
        alert("清空成功");
      } catch (/* error 表示默认回复记录清理异常。 */ error) {
