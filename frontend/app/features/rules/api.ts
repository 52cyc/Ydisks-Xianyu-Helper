...
import {
  contractClient,
  runContractRequest,
} from "../../../shared/api-contract/client";
import { collectionFrom, objectFrom } from "../../../shared/http/contract";
import {
  getFulfillmentProduct as readFulfillmentProduct,
  listFulfillmentInstances as readFulfillmentInstances,
  type FulfillmentInstance,
} from "../../../shared/api-contract/fulfillment";
export type * from "./models";
export type { FulfillmentInstance };

/** listFulfillmentInstances 为规则编辑器提供已配置货源摘要，隔离底层传输契约。 */
export const listFulfillmentInstances = (): Promise<FulfillmentInstance[]> =>
  readFulfillmentInstances();

/** getFulfillmentProduct 校验规则编辑器填写的单个外部商品。 */
export const getFulfillmentProduct = (instanceID: number, goodsID: number) =>
  readFulfillmentProduct(instanceID, goodsID);
...
export const getAccountDetails = async (
  ...
  runContractRequest(
    /* signal 控制规则页账号摘要读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/accounts/details", { signal }),
    options,
  );
...
export const getCards = async (
  ...
  runContractRequest(
    /* signal 控制规则页卡券读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/cards", { signal }),
    options,
...
export const getItems = async (
  ...
  runContractRequest(
    /* signal 控制规则页商品读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/items", {
...
// Rules - 自动化规则
const normalizeShippingRules = (rules: any[]): ShippingRule[] =>
  rules.map(
    /* 当前回调用于处理集合元素或接口响应。 */ (item: any) => ({
      id: String(item.id),
      name: item.name || "",
      ...
      card_group_id: Number(
        (item.actions || []).find(
          /* 当前回调用于处理集合元素或接口响应。 */ (a: any) =>
            a.action_type === "send_card",
        )?.card_id || 0,
      ),
      card_group_name:
        (item.actions || []).find(
      ...
      variants: (item.actions || [])
        ...
        .map(
          /* 当前回调用于处理集合元素或接口响应。 */ (action: any) => {
            ...
            let cfg: any = {};
            try {
              cfg = JSON.parse(action.config_json || "{}");
            } catch {}
            return {
              id: action.id ? String(action.id) : undefined,
              ...
              config_json: action.config_json || "{}",
              source_type:
                cfg.source_type === "external" ? "external" : "local",
              instance_id: Number(cfg.instance_id || 0),
              goods_ref: cfg.goods_id ? String(cfg.goods_id) : "",
              goods_id: Number(cfg.goods_id || 0),
              goods_name: cfg.goods_name || "",
              ...
              attach_fields: Array.isArray(cfg.attach_fields)
                ? cfg.attach_fields
                : [],
              product_verified: Number(cfg.goods_id || 0) > 0,
            };
          },
        ),
    }),
...
export const getShippingRules = async (): Promise<ShippingRule[]> => {
  ...
  const res = (await runContractRequest(
    /* signal 控制自动化规则列表读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/automation-rules", { signal }),
  )) as unknown;
  // rules 规则列表，用于当前 API 处理流程。
  const rules = collectionFrom<AutomationRuleResponse>(res, [
    "data",
    "rules",
    "items",
  ]);
...
export const getShippingRulesPage = async ({
  ...
  const response = (await runContractRequest(
    /* signal 控制自动化规则分页读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/automation-rules", {
        params: {
          query: {
            page: requestedPage,
            page_size: pageSize,
  ...
  const pageMeta =
    objectFrom<Partial<AutomationRulePageResponse>>(response, [
      "data",
      "result",
    ]) || {};
  // rules 是归一化后的自动化规则列表。
  const rules = normalizeShippingRules(
    collectionFrom<AutomationRuleResponse>(response, [
      "data",
      "rules",
      "items",
    ]),
  ...
  return {
    ...
    trigger_counts: Object.fromEntries(
      Object.entries(pageMeta.trigger_counts || {}).map(
        /* 当前回调用于处理集合元素或接口响应。 */ ([key, value]) => [
          key,
          Number(value),
        ],
      ),
...
const orderAutomationActions = (
  triggerType: string,
  actions: AutomationAction[],
) => {
  if (triggerType !== "order_paid") {
    return actions.map(
      /* 当前回调用于处理集合元素或接口响应。 */ (action, index) => ({
        ...action,
  ...
  const others = actions.filter(
    /* 当前回调用于处理集合元素或接口响应。 */ (action) =>
      action.action_type !== "send_card" &&
      action.action_type !== "confirm_shipment",
  );
  return [
    ...sendCards,
    ...others.map(
      /* 当前回调用于处理集合元素或接口响应。 */ (action, index) => ({
        ...action,
        sort_order: sendCards.length + index + 1,
      }),
    ),
    {
      action_type: "confirm_shipment" as const,
      enabled: true,
      sort_order: sendCards.length + others.length + 1,
    },
  ];
};
...
export const updateShippingRule = async (
...
): Promise<OperationResponse | MutationIDResponse> => {
  ...
  const preservedNonCardActions = (rule.actions || []).filter(
    /* 当前回调用于处理集合元素或接口响应。 */ (action) =>
      action.action_type !== "send_card" &&
      action.action_type !== "confirm_shipment",
  );
  ...
  const baseActions: AutomationAction[] =
    rule.variants && rule.variants.length > 0
      ? [
          ...rule.variants.map(
            /* 当前回调用于处理集合元素或接口响应。 */ (variant, index) => ({
              ...
              config_json: JSON.stringify({
                ...
                source_type:
                  variant.source_type === "external" ? "external" : "local",
                instance_id:
                  variant.source_type === "external"
                    ? Number(variant.instance_id || 0)
                    : 0,
                goods_id:
                  variant.source_type === "external"
                    ? Number(variant.goods_id || 0)
                    : 0,
                goods_name:
                  variant.source_type === "external"
                    ? variant.goods_name || ""
  ...
  return rule.id
    ? runContractRequest(
        /* signal 控制自动化规则更新请求的取消和超时。 */ (signal) =>
          contractClient.PUT("/api/v1/automation-rules/{rule_id}", {
            params: { path: { rule_id: String(rule.id) } },
            body: payload as never,
            signal,
          }),
      )
    : runContractRequest(
        /* signal 控制自动化规则创建请求的取消和超时。 */ (signal) =>
          contractClient.POST("/api/v1/automation-rules", {
            body: payload as never,
            signal,
          }),
      );
...
export const deleteShippingRule = async (
  ...
  runContractRequest(
    /* signal 控制自动化规则删除请求的取消和超时。 */ (signal) =>
      contractClient.DELETE("/api/v1/automation-rules/{rule_id}", {
...
export interface AutomationRunIssue {
  ...
  /** issue_kind 表示问题类型。 */ issue_kind:
    | "external_result_unknown"
    | "invalid_snapshot"
    | "rule_unavailable"
    | "partial_failure"
    | "execution_failed";
  /** allowed_resolutions 表示允许的解决方式。 */ allowed_resolutions: Array<
...
export const getAutomationIssues = async (): Promise<{
  /** runs 表示运行记录。 */ runs: AutomationRunIssue[];
  /** pending_tasks 表示待处理任务列表。 */ pending_tasks: DeferredAutomationIssue[];
}> => {
  // response 是兼容直接问题对象、data 包裹和 null 的自动化问题响应。
  const response = (await runContractRequest(
    /* signal 控制自动化问题读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/automation-issues", { signal }),
  )) as unknown;
  // result 是去除历史包裹后的自动化问题对象。
  const result =
    objectFrom<Partial<AutomationIssuesEnvelope>>(response, [
      "data",
      "result",
    ]) || {};
  return {
    runs: Array.isArray(result?.runs) ? result.runs : [],
    pending_tasks: Array.isArray(result?.pending_tasks)
...
export const resolveAutomationRun = async (
  id: number,
  resolution: "continue" | "retry" | "cancel",
): Promise<OperationResponse> =>
  runContractRequest(
    /* signal 控制自动化运行处理请求的取消和超时。 */ (signal) =>
      contractClient.POST("/api/v1/automation-runs/{run_id}/resolve", {
        params: { path: { run_id: String(id) } },
...
export const resolveDeferredAutomationTask = async (
  id: number,
  resolution: "retry" | "dismiss",
): Promise<OperationResponse> =>
  runContractRequest(
    /* signal 控制待处理自动化任务请求的取消和超时。 */ (signal) =>
      contractClient.POST(
        "/api/v1/automation-pending-tasks/{task_id}/resolve",
        {
          params: { path: { task_id: String(id) } },
          body: { resolution } as never,
          signal,
        },
      ),
...
const getKeywordRowsWithType = async (
  cookieId: string,
): Promise<KeywordRowPayload[]> => {
  // existing 已有规则，用于当前 API 处理流程。
  const existing = (await runContractRequest(
    /* signal 控制关键词规则读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/reply-rules/{cid}/typed", {
        params: { path: { cid: cookieId } },
        signal,
      }),
  )) as unknown;
  return collectionFrom<KeywordTypedResponse>(existing, [
    "data",
    "items",
    "rules",
...
export const getReplyRules = async (
  cookieId?: string,
): Promise<ReplyRule[]> => {
  if (!cookieId) return [];
...
export const updateReplyRule = async (
  ...
  return rule.id
    ? runContractRequest(
        /* signal 控制关键词规则更新请求的取消和超时。 */ (signal) =>
          contractClient.PUT("/api/v1/reply-rules/{cid}/typed/{id}", {
            params: { path: { cid: cookieId, id: String(rule.id) } },
            body: payload as never,
            signal,
          }),
      )
    : runContractRequest(
        /* signal 控制关键词规则创建请求的取消和超时。 */ (signal) =>
          contractClient.POST("/api/v1/reply-rules/{cid}/items", {
            params: { path: { cid: cookieId } },
            body: payload as never,
            signal,
          }),
...
export const deleteReplyRule = async (
  id: string,
  cookieId: string,
): Promise<OperationResponse> => {
  return runContractRequest(
    /* signal 控制关键词规则删除请求的取消和超时。 */ (signal) =>
      contractClient.DELETE("/api/v1/reply-rules/{cid}/typed/{id}", {
        params: { path: { cid: cookieId, id } },
        signal,
      }),
  );
};
...
export const getDefaultReplies = async (): Promise<
  Record<string, DefaultReplyResponse>
> => {
  const response = (await runContractRequest(
    /* signal 控制默认回复列表读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/default-replies", { signal }),
  )) as unknown;
  // replies 是兼容直接映射、data 包裹和 null 的默认回复索引。
  return (
    objectFrom<Record<string, DefaultReplyResponse>>(response, [
      "data",
      "replies",
      "items",
    ]) || {}
...
export const getDefaultReply = async (
  cookieId: string,
): Promise<DefaultReply> => {
  // result 接口响应结果，用于当前 API 处理流程。
  const response = (await runContractRequest(
    /* signal 控制默认回复读取的取消和超时。 */ (signal) =>
      contractClient.GET("/api/v1/default-replies/{cid}", {
        params: { path: { cid: cookieId } },
        signal,
      }),
  )) as unknown;
  ...
  const result =
    objectFrom<Partial<DefaultReplyResponse>>(response, ["data", "result"]) ||
    {};
  return {
    cookie_id: cookieId,
...
export const updateDefaultReply = async (
  cookieId: string,
  data: Partial<DefaultReply>,
): Promise<OperationResponse> => {
  return runContractRequest(
    /* signal 控制默认回复更新请求的取消和超时。 */ (signal) =>
      contractClient.PUT("/api/v1/default-replies/{cid}", {
        params: { path: { cid: cookieId } },
        body: {
...
export const deleteDefaultReply = async (
  cookieId: string,
): Promise<OperationResponse> => {
  return runContractRequest(
    /* signal 控制默认回复删除请求的取消和超时。 */ (signal) =>
      contractClient.DELETE("/api/v1/default-replies/{cid}", {
        params: { path: { cid: cookieId } },
        signal,
      }),
  );
...
export const clearDefaultReplyRecords = async (
  cookieId: string,
): Promise<OperationResponse> => {
  return runContractRequest(
    /* signal 控制默认回复记录清理请求的取消和超时。 */ (signal) =>
      contractClient.POST("/api/v1/default-replies/{cid}/clear-records", {
        params: { path: { cid: cookieId } },
        body: {} as never,
        signal,
      }),
