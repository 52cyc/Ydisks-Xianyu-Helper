        ...
        variants: (item.actions || [])
          ...
          .map(/* 当前回调用于处理集合元素或接口响应。 */ (action: any) => {
            ...
            return {
              ...
              source_type: cfg.source_type === 'external' ? 'external' : 'local',
              ...
              price_sync_enabled: cfg.price_sync_enabled === true || cfg.pending_price_enabled === true,
              stop_purchase_on_inversion: cfg.stop_purchase_on_inversion !== false,
              attach_json: JSON.stringify(cfg.attach || {}, null, 2),
              attach_fields: Array.isArray(cfg.attach_fields) ? cfg.attach_fields : [],
    ...
    const baseActions: AutomationAction[] = rule.variants && rule.variants.length > 0
      ? [...rule.variants.map(/* 当前回调用于处理集合元素或接口响应。 */ (variant, index) => ({
            ...
            config_json: JSON.stringify({
              ...
              source_type: variant.source_type === 'external' ? 'external' : 'local',
              instance_id: variant.source_type === 'external' ? Number(variant.instance_id || 0) : 0,
              goods_id: variant.source_type === 'external' ? Number(variant.goods_id || 0) : 0,
              goods_name: variant.source_type === 'external' ? variant.goods_name || '' : '',
              goods_type: variant.source_type === 'external' ? Number(variant.goods_type || 0) : 0,
              goods_price: variant.source_type === 'external' ? variant.goods_price || '' : '',
              safe_price: variant.source_type === 'external' ? variant.safe_price || '' : '',
              pending_price_enabled: variant.source_type === 'external' && variant.pending_price_enabled === true,
              fixed_markup: variant.source_type === 'external' ? variant.fixed_markup || '' : '',
              minimum_profit: variant.source_type === 'external' ? variant.minimum_profit || '' : '',
              profit_rate: variant.source_type === 'external' ? variant.profit_rate || '2.00' : '',
              price_sync_enabled: variant.source_type === 'external' && variant.price_sync_enabled === true,
              stop_purchase_on_inversion: variant.source_type === 'external' && variant.stop_purchase_on_inversion !== false,
              attach: variant.source_type === 'external' ? JSON.parse(variant.attach_json || '{}') : {},
              attach_fields: variant.source_type === 'external' ? variant.attach_fields || [] : [],
            }),
