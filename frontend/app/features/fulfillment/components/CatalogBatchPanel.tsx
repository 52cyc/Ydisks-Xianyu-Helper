...
/** CatalogBatchPanel 完成卡速售目录选品、统一加价、预检和批量上架。 */
export const CatalogBatchPanel: React.FC<CatalogBatchPanelProps> = ({ instances }) => {
  ...
  // instanceId、setInstanceId 保存本次选品使用的卡速售实例。
  ...
  // kasushouInstances 是首版允许进入批量上架流程的已启用卡速售实例。
  const kasushouInstances = instances.filter(/* instance 是当前待筛选的货源实例。 */ instance => instance.provider === "kasushou_v2" && instance.enabled);
  ...
  /** loadCategories 切换卡速售实例后读取目录并清空旧选品。 */
  const loadCategories = async (nextInstanceId: number): Promise<void> => {
    ...
    setLoading(true);
    try {
      // loadedCategories 是当前卡速售实例的完整目录树。
  ...
  const publishSelected = async (): Promise<void> => {
    ...
    setMessage("");
    try {
      // details 在真正创建批次前重新读取，避免使用过期价格、库存或状态。
      const details = await Promise.all([...selectedIds].map(/* goodsId 是当前待复核的货源商品标识。 */ goodsId => getFulfillmentProduct(instanceId, goodsId)));
      ...
      const file = new File([csv], "kasushou-products.csv", { type: "text/csv;charset=utf-8" });
  ...
  return (
    <section className="ios-card overflow-hidden rounded-2xl bg-white shadow-lg">
      <div className="border-b border-slate-100 p-5">
        <h3 className="flex items-center gap-2 font-extrabold text-slate-900"><PackagePlus className="h-5 w-5 text-sky-600" /> 卡速售批量上架</h3>
      ...
      <div className="grid gap-4 p-5 md:grid-cols-3">
        ...
        <label className="space-y-1 text-sm font-bold text-slate-700">卡速售实例
          <select className="ios-input w-full rounded-xl" value={instanceId} onChange={/* instanceChangeHandler 切换实例并重新加载目录。 */ event => void loadCategories(Number(event.target.value))}><option value={0}>请选择</option>{kasushouInstances.map(/* instance 是当前卡速售实例选项。 */ instance => <option key={instance.id} value={instance.id}>{instance.name}</option>)}</select>
        </label>
