...
export function buildFulfillmentBatchCSV(products: FulfillmentProduct[], instanceId: number, accountId: string, profitRate: number): string {
  ...
  if (instanceId <= 0) throw new Error("请选择卡速售实例");
  ...
  const headers = [
    ...
    "external_goods_type", "external_safe_price", "external_profit_rate", "external_price_sync_enabled",
  ...
  for (const /* product 是当前转换为闲鱼商品的单规格货源商品。 */ product of products) {
    ...
    // quantity 使用货源库存并限制为闲鱼批量发布的合理上限。
    const quantity = Math.max(1, Math.min(product.stock_num > 0 ? product.stock_num : 1, 999));
    // values 与 headers 顺序一一对应，供现有批量预检直接消费。
    const values: Array<string | number | boolean> = [
      accountId, product.goods_name, description, markedUpPrice(product.goods_price || "", profitRate), quantity,
      "free", product.goods_img, true, instanceId, product.id, product.goods_name, product.goods_type,
      product.can_price ? (product.goods_price || "") : "", profitRate.toFixed(2), true, true,
    ];
