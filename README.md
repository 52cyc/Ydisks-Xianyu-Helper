...
- 付款后发货、砍价分阶段免拼、评价赠品、超时求评价等自动化流程
...
| 自动化中心 | 拍下未付款自动改价、付款后自动发货、砍价分阶段免拼、评价后赠品、超时未评价提醒、失败恢复与幂等检查点 |
| 账号自动任务 | 基于本地已记录确认收货事件执行自动评价、按北京时间每日擦亮、手动立即执行、执行记录与幂等保护 |
...
| AI 回复 | OpenAI 兼容 API、模型发现、端到端连接测试、自定义提示词、议价轮次和让价范围控制 |
...
| `XIANYU_PENDING_SHIP_CATCHUP` | 启用 | 设为 `0` 可停止待发货补偿扫描与检查点续跑 |
| `XIANYU_CONTINUE_AFTER_UNCERTAIN` | 启用 | 设为 `0` 可在消息动作结果不确定后保持熔断 |
