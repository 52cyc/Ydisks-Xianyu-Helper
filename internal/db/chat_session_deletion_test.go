...
// TestMultiDBChatSessionDeletionMigration 验证三方言 48 迁移的字段、列表索引和可回退结构保持一致。
	...
	for _, target := range allTestTargets(t) {
		t.Run(target.name, func(t *testing.T) {
			...
			if !columnExistsForDialect(t, target.store.DB, target.dialect, "chat_sessions", "user_hidden_at") ||
				...
				t.Fatal("migration 48 deletion columns are missing")
			}
			...
			// downErr 回滚到 fork 已发布的 47，撤销上游顺延后的 48-51，验证旧列表索引和字段结构可以恢复。
			if downErr := goose.DownTo(target.store.DB, "migrations/"+subdir, 47); downErr != nil {
				t.Fatal(downErr)
			}
			if columnExistsForDialect(t, target.store.DB, target.dialect, "chat_sessions", "user_hidden_at") ||
				columnExistsForDialect(t, target.store.DB, target.dialect, "chat_sessions", "messages_cleared_at") {
			...
			// upErr 再次应用 48-51，验证回滚没有遗留重名索引或不可重建结构。
