...
// CountByTriggerForUser 返回同一筛选条件下各触发类型的规则数量；统计不受分页影响，确保与 total 使用同一数据集。
...
// Match 查询某事件可触发的规则：商品级优先，否则回退到账号级；付款发货还要求明确确认适用于全部商品。
		...
		WHERE id=? AND user_id=? AND NOT EXISTS (
			SELECT 1 FROM automation_runs ar WHERE ar.rule_id=automation_rules.id
			  AND (ar.status IN ('running','needs_review') OR ar.action_started<>0
			    OR (ar.status='failed' AND
			      ((ar.attempt_count<3 AND ((ar.sent_count=0 AND ar.error_message NOT LIKE '[no_retry]%')
			        OR ar.error_message LIKE '[safe_retry]%'))
			       OR (ar.attempt_count<? AND ar.error_message LIKE '[external_wait]%')))))`, ruleID, userID, externalWaitMaxAttempts)
	if err != nil {
		return err
	}
	if // n 用于本次流程后续判断的n
	...
	res, err := execer.ExecContext(ctx, `UPDATE automation_runs
	 ...
	 WHERE rule_id=? AND trigger_key=? AND cookie_id=? AND COALESCE(order_id,'')=?
	   AND ((status='running' AND action_started=0 AND (lease_expires_at=0 OR lease_expires_at<?))
	        OR (status='failed' AND action_started=0 AND next_retry_at<=?
	            AND ((attempt_count<3 AND ((sent_count=0 AND error_message NOT LIKE '[no_retry]%') OR error_message LIKE '[safe_retry]%'))
	                 OR (attempt_count<? AND error_message LIKE '[external_wait]%'))))`,
		leaseExpiresAt, run.RuleID, run.TriggerKey, run.CookieID, run.OrderID, now, now, externalWaitMaxAttempts)
	if err != nil {
		return 0, false, err
	}
	...
	res, err := a.DB.ExecContext(ctx, `UPDATE automation_runs
		SET status='running',error_message='',lease_expires_at=?,next_retry_at=0,attempt_count=attempt_count+1,updated_at=CURRENT_TIMESTAMP
		 ...
		 OR (status='failed' AND next_retry_at<=?
		     AND ((attempt_count<3 AND ((sent_count=0 AND error_message NOT LIKE '[no_retry]%') OR error_message LIKE '[safe_retry]%'))
		          OR (attempt_count<? AND error_message LIKE '[external_wait]%'))))`, leaseExpiresAt, runID, now, now, externalWaitMaxAttempts)
	if err != nil {
		return false, err
	}
...
SELECT id,rule_id,cookie_id,item_id,order_id,buyer_id,chat_id,trigger_type,trigger_key,
 ...
 WHERE (status='running' AND (lease_expires_at=0 OR lease_expires_at<?))
	    OR (status='failed' AND action_started=0 AND next_retry_at<=?
	        AND ((attempt_count<3 AND ((sent_count=0 AND error_message NOT LIKE '[no_retry]%') OR error_message LIKE '[safe_retry]%'))
	             OR (attempt_count<? AND error_message LIKE '[external_wait]%')))
	 ORDER BY updated_at,id LIMIT ?`, now, now, externalWaitMaxAttempts, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// RecoverDefinitelyUnsentReviewRuns 恢复旧版本把“发送前没有 WS 连接”误判成结果不确定的求评价运行。
// 这些记录 sent_count=0 且错误发生在调用发送接口前，可安全清除 action_started 并进入现有失败重试流程。
