...
func TestMultiDB_DeletedAutomationRulesUpgrade(t *testing.T) {
	...
	for _, target := range allTestTargets(t) {
		...
		t.Run(target.name, func(t *testing.T) {
			...
			// err 保存回退到清理迁移之前的错误；45 用于验证 46 守卫回填先于 47 删除。
			...
			for _, scenario := range []struct {
				...
				t.Run(scenario.name, func(t *testing.T) {
					...
					if scenario.status != "" {
						// err 构造升级前执行历史，后续 46 必须先留下独立订单守卫。
					...
					// err 先执行 46 回填旧执行守卫，再验证 47 升级不会随历史运行删除这些证据。
					if err := goose.UpTo(store.DB, "migrations/"+subdir, 46); err != nil {
						t.Fatal(err)
					}
					// err 保存从 46 执行完整启动迁移的错误。
					...
					// err 回退到守卫回填之前，下一场景仍从本地 45 模拟完整升级链。
					if err := goose.DownTo(store.DB, "migrations/"+subdir, 45); err != nil {
						t.Fatal(err)
					}
				})
...
func TestMultiDB_DeletedAutomationRulesSharedCard(t *testing.T) {
	...
	for _, target := range allTestTargets(t) {
		...
		t.Run(target.name, func(t *testing.T) {
			...
			goose.SetBaseFS(migrationsFS)
			// err 保存退回清理迁移前 46 版本的错误。
			if err := goose.DownTo(store.DB, "migrations/"+subdir, 46); err != nil {
				t.Fatal(err)
			}
