...
// TestMultiDB_OrderOwnershipLookupIndexes 验证 t 中各可用方言的 46 迁移创建索引、回滚删除索引及再次升级重建索引。
func TestMultiDB_OrderOwnershipLookupIndexes(t *testing.T) {
	...
	for _, target := range allTestTargets(t) {
		...
		t.Run(target.name, func(t *testing.T) {
			...
			// err 保存回滚至 45 的结果；46 的审计表和查询索引必须同时退出。
			if err := goose.DownTo(target.store.DB, "migrations/"+subdir, 45); err != nil {
				t.Fatal(err)
			}
			...
			if tableExistsForDialect(t, target.store.DB, target.dialect, "order_ownership_repairs") {
				t.Fatal("回滚 46 后仍残留归属审计表")
			}
			// err 保存重新升级至 46 的结果，用于发现 Down 遗留的重名索引。
			if err := goose.UpTo(target.store.DB, "migrations/"+subdir, 46); err != nil {
				t.Fatal(err)
			}
			assertOwnershipLookupIndexes(t, target.store.DB, target.dialect, true)
			if !tableExistsForDialect(t, target.store.DB, target.dialect, "order_ownership_repairs") {
				t.Fatal("重新升级 46 后未恢复归属审计表")
			}
		})
	}
}

// TestMultiDB_OrderAutomationGuardsBackfill 验证 t 中 46 回填所有状态的去重执行痕迹，规则级联删除后仍阻止恢复。
// 守卫无外键且无秘密；回滚 46 必须删除守卫表，重新升级必须恢复 schema。
func TestMultiDB_OrderAutomationGuardsBackfill(t *testing.T) {
	...
	for _, target := range allTestTargets(t) {
		...
		t.Run(target.name, func(t *testing.T) {
			...
			goose.SetBaseFS(migrationsFS)
			// err 保存回退到没有守卫表的 45 的失败，以复现合并后的真实升级入口。
			if err := goose.DownTo(store.DB, "migrations/"+subdir, 45); err != nil {
				t.Fatal(err)
			}
			...
			if err := goose.UpTo(store.DB, "migrations/"+subdir, 46); err != nil {
				t.Fatal(err)
			}
