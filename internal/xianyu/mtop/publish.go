...
func (c *ClientImpl) uploadPublishImage(ctx context.Context, cookiesStr string, img PublishImage) (uploadedImage, string, error) {
	...
	if img.ContentType == "" {
		img.ContentType = http.DetectContentType(img.Data)
	}
	// filename、filenameErr 保存本次上传独占的短随机文件名和生成错误，禁止透传来源 URL 中的超长名称。
	filename, filenameErr := randomPublishImageFilename(cryptorand.Reader, img.ContentType, img.Data)
	if filenameErr != nil {
		return uploadedImage{}, cookiesStr, filenameErr
	}
