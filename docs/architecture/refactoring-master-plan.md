...
- 2026-09-18（fork 合并兼容）：上游 v1.0.13 的账号自动砍价与砍价免拼阶段迁移原使用 00050/00051，与本 fork 已发布的用户认证版本和聊天角色迁移冲突。合并后在 SQLite、MySQL、PostgreSQL 中统一顺延为 `00055_account_auto_bargain` 和 `00056_bargain_free_shipping_stage`；保留本 fork 00050–00054 既有语义，并通过迁移测试验证最终账本与两项上游结构。

- 2026-09-16（账号间商品克隆规则与多卡密展示，本地未发布）：账号间克隆快照通过既有批量预检携带源账号和源商品双键，目标商品远端发布成功后复制当前用户下所有明确绑定源商品的自动化规则；目标规则替换账号和商品标识，保留开关、优先级、动作顺序、卡密组、发货模板与通知配置，并使用源规则标识保证本地恢复幂等。账号通用规则、历史运行、订单、报价和延期任务不复制。外部货源一次返回多份卡密时，`{delivery_content}` 按“卡密 1、卡密 2”分行渲染，单份内容保持原格式。范围限于现有克隆 CSV 快照、批量发布本地收口、自动化规则适配和外部交付文本；不增加 HTTP/OpenAPI 路由、数据库 schema，不修改冻结 CAPTCHA。

- 2026-09-16（外部履约恢复时延与改价自然收口，本地未发布）：外部货源首次采购返回 `waiting/processing` 后不再立即查单，运行在五秒后进入渐进退避；恢复任务由独立五秒循环领取，不依赖分钟级综合扫描，调度器关闭时等待该循环退出。恢复始终查询持久化的原外部订单号，供应站暂时返回不存在时继续等待且禁止再次采购。待付款改价若在重试期间因买家付款变为“当前订单状态不支持改价”，按流程自然结束并降为 Info；付款流程已经并发接管动态保护价快照时保留 `adjusted` 结果，不再报告“报价不存在或已经收口”。范围限于履约应用/适配、自动化调度与 MTOP 改价分类，不改变 HTTP/OpenAPI、数据库 schema、六阶段状态或冻结 CAPTCHA。

- 2026-09-15（卡速售限流与 fork 合并兼容，本地未发布）：卡速售 v2 客户端在统一 HTTP 边界按归一化站点和商户共享串行队列，目录、列表、详情、询价、后台价格同步、采购与查单均不并发，相邻真实请求启动间隔至少 3.5 秒，保守满足每 10 秒最多 3 次；等待可随 Context 取消，API Key 不进入限流键。卡密商品详情跳过仅直充需要的 `goods/attach`，不改变直充字段、采购幂等和不确定结果禁止重试语义。同步合并上游 v1.0.12，组合保留本地直充收货字段、价格同步与上游砍价免拼、待发货补偿、Session 恢复边界；将上游新增的超长动作循环拆分为延迟/聊天输入准备与执行两个职责，不改业务分支。上游合并后的 automation、adapter、engine、MTOP、renewal、db 全包测试通过；卡速售普通与 race、自动化普通与受影响分支定向 race、vet、架构与 API 门禁通过。全量 Go 测试仅 `cmd/tray` 因本机 systray 图形编译环境缺失失败，其余包通过；全量 automation race 在 bcrypt/SQLite 迁移压力中 10 分钟超时，受影响用例定向 race 通过。`make comments` 仍被 138 条未触及声明的既有注释债务阻断，本次限流与职责拆分声明无新增项。未执行真实供应站、真实闲鱼账号、MySQL/PostgreSQL 或桌面包验证；不修改冻结 CAPTCHA、HTTP/OpenAPI 或数据库 schema。

- 2026-09-15（货源商品不可采购保护价）：后台商品页价格同步在供应站成功返回商品但明确 `CanBuy=false` 时，与商品删除一样通过既有闲鱼改价和本地回写路径设为 `9999.00`，并在日志中区分 `missing` 与 `unavailable`。限流、TLS 超时、鉴权或供应站系统错误仍保留原价；待付款改价和实际采购路径仍拒绝不可采购商品。新增确定性和 race 回归；受影响自动化包全量测试、适配层全量复跑、vet、架构与 API 门禁通过。`make comments` 仅被 138 条未触及文件的既有注释债务阻断，本次修改文件无新增项。

- 2026-09-14（fork 合并兼容）：上游 v1.0.11 的账号任务重试上限和自动确认发货迁移原使用 00048/00049，与本分支已发布的 00048–00052 冲突。合并后在 SQLite、MySQL、PostgreSQL 中统一顺延为 `00053_account_task_retry_limit` 和 `00054_account_auto_consign`；迁移测试验证本地既有语义与上游新增字段均完整存在，禁止重复迁移号导致任一方升级被跳过。

- 2026-09-06（fork 合并兼容）：上游 v1.0.10 的订单归属修复和已删除自动化规则清理原使用迁移号 00042/00043，与本分支已经发布的 00042–00045 冲突。合并后在 SQLite、MySQL、PostgreSQL 中统一顺延为 `00046_order_ownership_repairs` 和 `00047_deleted_automation_rules_cleanup`；历史条目保留上游原编号作为来源记录，当前分支实际升级顺序以 00046/00047 为准。迁移测试从本分支 00045 基线验证 00046 回填先于 00047 清理，禁止因同号账本误判而跳过任一方数据变更。
